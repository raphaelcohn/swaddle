#!/usr/bin/env sh
set -e
set -u

if [ -t 1 ]; then
	printf '%s\n' 'This script will install the apt repository swaddle' 'It will change your apt keys, create or replace /etc/apt/sources.list.d/00swaddle.sources.list, install apt-transport-https and update apt.' 'Press the [Enter] key to continue.'
	read -r garbage
fi

sudo -p "Password for %p to allow root to update from new sources before installing apt-transport-https: " apt-get --quiet update
sudo -p "Password for %p to allow root to  apt-get install apt-transport-https (missing in Debian default installs)" apt-get install apt-transport-https

temporaryKeyFile="$(mktemp --tmpdir swaddle.key.XXXXXXXXX)"
trap 'rm -rf "$temporaryKeyFile"' EXIT HUP INT QUIT TERM
cat >"$temporaryKeyFile" <<EOF
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1.4.5 (GNU/Linux)


=2vnB
-----END PGP PUBLIC KEY BLOCK-----
EOF
sudo -p "Password for %p is required to allow root to install repository 'swaddle' public key to apt: " apt-key add "$temporaryKeyFile"

echo 'deb https://raphaelcohn.github.io/swaddle/download/apt swaddle multiverse' | sudo -p "Password for %p is required to allow root to install repository 'swaddle' apt sources list to '/etc/apt/sources.list.d/00swaddle.sources.list': " tee /etc/apt/sources.list.d/00swaddle.list >/dev/null
sudo -p "Password for %p to allow root to update from new sources: " apt-get --quiet update
